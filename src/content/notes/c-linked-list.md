---
title: "链表"
description: "本章导览 链表是 C 语言中最重要的动态数据结构之一。与数组的连续存储不同，链表通过指针将散落在内存各处的结点串联起来，可以在运行时灵活地增删数据。 本章涵盖：链表基本概念 → 静态链表 → 动态链表的创建 → 链表的遍历输出 → 综合练习 相关基础：c-pointers、c-custom-types"
language: "zh-CN"
date: 2026-07-11
