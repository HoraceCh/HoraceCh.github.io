---
title: "树与二叉树"
description: "树（Tree） 是 n(n>=0) 个结点的有限集。n=0 时称为空树。 在任意一颗非空树中： 1. 有且仅有一个特定的称为根(Root)的结点。 2. 当n>1时，其余节点可分为m（m>0）个互不相交的有限集T1,T2,…,Tm，其中每个集合本身又是一棵树，并且称为根的子树。"
language: "zh-CN"
date: 2026-03-13
...
一棵高度为 h，且含有 2^h-1 个结点的二叉树称为满二叉树，即树中的每层都含有最多的结点。满二叉树的叶子结点都集中在二叉树的最下一层，并且除叶子结点之外的每个结点度数均为 2 。可以对满二叉树按层序编号:约定编号从根结点(根结点编号为1)起，自上而下，自左向右。这样，每个结点对应一个编号,对于编号为i的结点，若有双亲，则其双亲为i/2,若有左孩子，则左孩子为2i;若有右孩子，则右孩子为2i+1。![满二叉树图示](/notes-assets/tree-binary-tree/asset-1tycfh.png)
#### 3. 完全二叉树
...
1. [前序遍历](#前序遍历) 根结点 > 左子树 > 右子树
2. [中序遍历](#中序遍历) 左子树> 根结点 > 右子树
3. [后序遍历](#后序遍历) 左子树 > 右子树 > 根结点
4. [层序遍历](#层次遍历) 仅仅需按层次遍历就可以
