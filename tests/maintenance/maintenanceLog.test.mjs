...
import { newCorrelationId, rebuildIndex, validateRepository } from '../../tools/maintenance-log.mjs';
...
test('rebuilds the generated index deterministically from canonical records', async (t) => {
  ...
  const firstFile = await readFile(path.join(maintenance, 'index.md'), 'utf8');
  const second = await rebuildIndex(root);
  const secondFile = await readFile(path.join(maintenance, 'index.md'), 'utf8');

  assert.equal(first.output, second.output);
  assert.equal(firstFile, secondFile);
  assert.match(secondFile, /Generated by tools\/maintenance-log\.mjs/);
  assert.match(secondFile, /\[WEB-20260728-0001\]\(events\/WEB-20260728-0001\.md\)/);
  assert.match(secondFile, /Legacy System Maintenance Log v1/);
});
