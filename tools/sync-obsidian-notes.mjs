...
async function main() {
  ...
  const headingContracts = buildHeadingContracts(records);
...
function buildHeadingContracts(records) {
  const contracts = new Map();

  for (const record of records) {
    let body = record.body.replace(/\r\n/g, '\n');
    body = removeDataviewBlocks(body, record.relativePath, []).body;
    body = removeObsidianComments(body);
    body = convertCallouts(body);
    body = convertHighlights(body);
    body = normalizeCodeFenceInfoStrings(body);
    body = normalizeReadableMath(body, record.relativePath, []);
    body = normalizeBodyHeadingDepth(body, record.relativePath, []);

    const slugger = new GithubSlugger();
    const headings = collectRendererHeadings(body).map((heading) => ({
      ...heading,
      id: slugger.slug(heading.text),
      compact: fragmentCompact(heading.text),
      asciiTokens: fragmentAsciiTokens(heading.text),
    }));
    contracts.set(record.slug, { headings, record });
  }

  return contracts;
}

function collectRendererHeadings(body) {
  const headings = [];
  ...
  for (const line of body.split('\n')) {
    ...
    const text = rendererHeadingText(heading[2].replace(/[ \t]+#+[ \t]*$/, ''));
    if (text) {
      headings.push({ text });
    }
  }
...
function rendererHeadingText(value) {
  const inlineCode = [];
  return collapseWhitespace(
    String(value)
      .replace(/(`+)([^`\n]*?)\1/g, (_match, _delimiter, code) => `\uE000${inlineCode.push(code) - 1}\uE001`)
      .replace(/!\[\[([^\]]+)\]\]/g, (_match, rawTarget) => {
        const target = parseObsidianLink(rawTarget);
        return target.alias || target.heading || assetAltText(target.path);
      })
      .replace(/\[\[([^\]]+)\]\]/g, (_match, rawTarget) => {
        const target = parseObsidianLink(rawTarget);
        return target.alias || target.heading || path.basename(target.path);
      })
      .replace(/!\[([^\]]*)\]\([^)]*\)/g, '$1')
      .replace(/\[([^\]]+)\]\([^)]*\)/g, '$1')
      .replace(/<[^>]+>/g, '')
...
function fragmentCompact(value) {
  return rendererHeadingText(value).normalize('NFKC').toLowerCase().replace(/[^\p{L}\p{N}]+/gu, '');
}

function fragmentAsciiTokens(value) {
  return rendererHeadingText(value).toLowerCase().match(/[a-z0-9]+/g) || [];
}
...
async function convertRecord({
  ...
  const outFile = path.join(outDir, `${record.slug}.md`);
  const existingFrontmatter = await readGeneratedFrontmatter(outFile);
  const frontmatter = normalizeFrontmatter({ record, warnings, noteIndex, existingFrontmatter });
...
function normalizeFrontmatter({ record, warnings, noteIndex, existingFrontmatter }) {
  ...
  const date = normalizeDate(
    firstString(fm.date, fm.created, fm.created_at, fm.createdAt),
    normalizeDate(existingFrontmatter?.date, null),
  );
  const updated = normalizeDate(firstString(fm.updated, fm.modified, fm.lastmod, fm.last_modified), null);
  const description = normalizeDescription(
    firstString(fm.description, fm.summary, fm.excerpt),
    bodyWithoutDataview,
    warnings,
    record.relativePath,
  );
  const language = normalizeLanguage(
    firstString(fm.language, fm.lang),
    `${record.title}\n${bodyWithoutDataview}`,
    warnings,
    record.relativePath,
  );
...
async function readGeneratedFrontmatter(file) {
  try {
    const raw = await readFile(file, 'utf8');
    if (!raw.includes(GENERATED_MARKER)) {
      return null;
    }
    const { frontmatter } = splitFrontmatter(raw);
    return parseFrontmatter(frontmatter, slash(file), []);
  } catch (error) {
...
async function convertBody({
  ...
  nextBody = normalizeReadableMath(nextBody, record.relativePath, warnings);
  ...
  nextBody = rewritePublishedNoteFragments(nextBody, record, headingContracts, warnings);
  return nextBody;
}
...
function convertCallouts(body) {
  return body.replace(
    /^((?:[ \t]{0,3}>[ \t]?)+)[ \t]*\[!([A-Za-z0-9_-]+)\][+-]?[ \t]*(.*)$/gm,
    (_match, rawPrefix, kind, rawTitle) => {
      const prefix = `${'> '.repeat(blockquoteDepth(rawPrefix)).trimEnd()} `;
      const title = rawTitle.trim().replace(/^>[ \t]*/, '');
      const fence = title.match(/^(`{3,}|~{3,})(.*)$/);
      if (fence) {
        return `${prefix}**${titleCase(kind)}**\uE002\n${prefix}${fence[1]}${fence[2]}`;
      }
      const label = title ? `${titleCase(kind)}: ${title}` : titleCase(kind);
      return `${prefix}**${label}**\uE002`;
    },
  )
    .replace(/\uE002\n([^\n]*\S) {2,}(?=\n|$)/g, '\n$1\\')
...
function normalizeCodeFenceInfoStrings(body) {
  ...
  return lines
    .map((line) => {
      ...
      if (activeFence?.blockquoteDepth > blockquoteDepth(content)) {
        activeFence = null;
      }
      if (activeFence) {
...
function normalizeReadableMath(body, file, warnings) {
  const lines = body.split('\n');
  const output = [];
  let activeFence = null;
  let prose = [];
  ...
  const flushProse = () => {
    if (prose.length > 0) {
      output.push(normalizeMathOutsideInlineCode(prose.join('\n'), file, warnings));
      prose = [];
    }
  };
...
function normalizeMathOutsideInlineCode(value, file, warnings) {
  const destinations = [];
  const protectedValue = value.replace(/(!?\[[^\]\n]*\]\()([^)\n]+)(\))/g, (_match, opening, destination, closing) => {
    const token = `\uE000${destinations.length}\uE001`;
  ...
  while (index < protectedValue.length) {
    if (protectedValue[index] !== '`') {
      index += 1;
      continue;
    }

    let runLength = 1;
    while (protectedValue[index + runLength] === '`') {
      runLength += 1;
    }
    const delimiter = '`'.repeat(runLength);
    const closing = protectedValue.indexOf(delimiter, index + runLength);
    if (closing < 0) {
      break;
    }

    output += normalizeMathDelimiters(protectedValue.slice(proseStart, index), file, warnings);
    output += protectedValue.slice(index, closing + runLength);
    index = closing + runLength;
    proseStart = index;
  }

  output += normalizeMathDelimiters(protectedValue.slice(proseStart), file, warnings);
  output = output.replace(/\uE000(\d+)\uE001/g, (_match, destinationIndex) => destinations[Number(destinationIndex)]);
  ...
  if (originalLines.length === outputLines.length) {
    output = outputLines
      .map((line, lineIndex) => {
        return line !== originalLines[lineIndex] && / {2,}$/.test(originalLines[lineIndex])
          ? line.replace(/[ \t]+$/, '\\')
          : line;
      })
      .join('\n');
...
function normalizeMathDelimiters(value, file, warnings) {
  return value
    .replace(/\$\$([\s\S]*?)\$\$/g, (_match, expression) => readableMathExpression(expression, file, warnings))
    .replace(/(^|[^\\$])\$([^\n$]+?)\$/g, (_match, prefix, expression) => {
      return `${prefix}${readableMathExpression(expression, file, warnings)}`;
    });
}

function readableMathExpression(value, file, warnings) {
  ...
  const formattingCommands = new Set(['text', 'mathrm', 'mathbf', 'mathit', 'operatorname']);
  const unsupported = unique(
    Array.from(String(value).matchAll(/\\([A-Za-z]+)/g), (match) => match[1]).filter(
      (command) => !supportedCommands.has(command) && !formattingCommands.has(command) && command !== 'frac' && command !== 'sqrt' && command !== 'log',
    ),
  );
  ...
  if (unsupported.length > 0) {
    warnings.push({
      file,
      message: `Degraded unsupported TeX command(s) to plain text: ${unsupported.map((command) => `\\${command}`).join(', ')}.`,
      type: 'math',
    });
  }
  ...
  result = result.replace(/\\(?:text|mathrm|mathbf|mathit|operatorname)\s*\{([^{}]+)\}/g, '$1');
  result = result.replace(/\\log\b/g, 'log');
  for (const [command, replacement] of supportedCommands) {
...
function normalizeBodyHeadingDepth(body, file, warnings) {
  ...
  for (const heading of headings) {
    ...
    lines[heading.index] = `${heading.prefix}#${heading.marker}${heading.remainder.replace(/[ \t]+$/, '')}`;
  }
...
function noteMarkdownLink({ target, record, noteIndex, warnings, vault, resolveWikilinksFromVaultRoot }) {
  ...
  if (!target.path && target.heading) {
    return `[${label}](#${githubSlug(target.heading)})`;
  }
  ...
  const anchor = target.heading ? `#${githubSlug(target.heading)}` : '';
  return `[${label}](/notes/${resolution.record.slug}/${anchor})`;
}

function rewritePublishedNoteFragments(body, record, headingContracts, warnings) {
  const lines = [];
  let activeFence = null;
  ...
  for (const line of body.split('\n')) {
    ...
    const rewritten = rewriteFragmentLinksOutsideInlineCode(line, record, headingContracts, warnings);
    const publishedFragment = /\]\((?:\/notes\/[^)\s#]+\/?)?#[^)]*\)/.test(line);
    lines.push((rewritten !== line || publishedFragment) && / {2,}$/.test(line) ? rewritten.replace(/[ \t]+$/, '\\') : rewritten);
  }

  return lines.join('\n');
}

function rewriteFragmentLinksOutsideInlineCode(line, record, headingContracts, warnings) {
  let output = '';
  let proseStart = 0;
  let index = 0;
  ...
  while (index < line.length) {
    ...
    output += rewriteFragmentLinks(line.slice(proseStart, index), record, headingContracts, warnings);
    output += line.slice(index, closing + runLength);
    index = closing + runLength;
    proseStart = index;
  }

  return output + rewriteFragmentLinks(line.slice(proseStart), record, headingContracts, warnings);
}

function rewriteFragmentLinks(value, record, headingContracts, warnings) {
  return value.replace(/(^|[^!])\[([^\]\n]+)\]\(([^)\n]+)\)/g, (match, prefix, label, destination) => {
    const hashIndex = destination.indexOf('#');
    if (hashIndex < 0) {
      return match;
    }

    const base = destination.slice(0, hashIndex);
    const rawFragment = destination.slice(hashIndex + 1);
    let targetSlug = record.slug;
    if (base) {
      const noteRoute = base.match(/^\/notes\/([^/?#]+)\/?(?:\?[^#]*)?$/);
      if (!noteRoute) {
        return match;
      }
      targetSlug = decodeUriPath(noteRoute[1]);
    }

    const contract = headingContracts.get(targetSlug);
    if (!contract) {
      return match;
    }

    const requested = decodeUriPath(rawFragment || rendererHeadingText(label));
    const resolved = resolveHeadingFragment(contract, requested);
    if (!resolved) {
      warnings.push({
        file: record.relativePath,
        message: `Unresolved published Note fragment "#${requested}" targeting "${targetSlug}".`,
        type: 'fragment',
      });
      return match;
    }

    return `${prefix}[${label}](${base}#${resolved})`;
  });
}

function resolveHeadingFragment(contract, requested) {
  const query = rendererHeadingText(requested);
  if (!query) {
    return null;
  }

  const queryLower = query.toLowerCase();
  const querySlug = githubSlug(query);
  const queryCompact = fragmentCompact(query);
  const queryCompacts = fragmentEquivalentCompacts(queryCompact);
  const queryTokens = fragmentAsciiTokens(query);
  const exactId = contract.headings.find((heading) => heading.id.toLowerCase() === queryLower || heading.id === querySlug);
  if (exactId) {
    return exactId.id;
  }

  const exactText = contract.headings.find(
    (heading) => heading.text.toLowerCase() === queryLower || queryCompacts.includes(heading.compact),
  );
  if (exactText) {
    return exactText.id;
  }

  const prefixMatches = contract.headings.filter(
    (heading) => queryCompact && (heading.compact.startsWith(queryCompact) || queryCompact.startsWith(heading.compact)),
  );
  ...
  if (queryTokens.length > 0) {
    const tokenMatches = contract.headings.filter((heading) => containsTokenSequence(heading.asciiTokens, queryTokens));
    if (tokenMatches.length === 1) {
      return tokenMatches[0].id;
    }
  }

  const prefixScores = contract.headings
    .map((heading) => ({ heading, score: commonPrefixLength(heading.compact, queryCompact) }))
    .filter((candidate) => candidate.score >= 2 && candidate.score / Math.min(candidate.heading.compact.length, queryCompact.length) >= 0.5)
    .sort((a, b) => b.score - a.score);
  if (prefixScores.length > 0 && (prefixScores.length === 1 || prefixScores[0].score > prefixScores[1].score)) {
    return prefixScores[0].heading.id;
  }

  return null;
}

function fragmentEquivalentCompacts(value) {
  const equivalents = [value];
  if (value.includes('层序')) {
    equivalents.push(value.replace(/层序/g, '层次'));
  }
  if (value.endsWith('类型')) {
    const base = value.slice(0, -2);
    equivalents.push(`${base.endsWith('型') ? base : `${base}型`}数据`);
  }
  return unique(equivalents);
}

function containsTokenSequence(haystack, needles) {
  let needleIndex = 0;
...
function normalizeDescription(value, body, warnings, file) {
  const candidates = [stringValue(value), ...descriptionParagraphs(body)];

  for (const [candidateIndex, candidate] of candidates.entries()) {
    const candidateWarnings = candidateIndex === 0 ? warnings : [];
    const description = plainTextDescription(normalizeReadableMath(candidate, file, candidateWarnings));
    if (description) {
...
function normalizeLanguage(value, content, warnings, file) {
  ...
  if (language) {
    ...
    warnings.push({
      file,
      message: `Normalized unsupported language "${value}" to "en".`,
      type: 'frontmatter',
    });
    return 'en';
  }
  ...
  const latinWordCount = (prose.match(/[A-Za-z]+(?:['’-][A-Za-z]+)*/g) || []).length;
  return hanCount >= 20 && hanCount > latinWordCount ? 'zh-CN' : 'en';
}
...
function descriptionParagraphs(body) {
  return String(body)
    .replace(/^(?:[ \t]{0,3})(`{3,}|~{3,})[^\n]*\n[\s\S]*?^\1[ \t]*$/gm, '')
    .split(/\n\s*\n/)
